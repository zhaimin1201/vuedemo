<template>
  <div class="visi">
    <div class="visicon image-com">
      <header class="head-tab">
        <ul>
          <li class="back-con"><span class="back" @click="goBack()">返回</span></li>
          <li class="visodata" >
          ...
          <li class="visimess" >
            <img alt="" src="../../images/04more@2x.png" class="image-com" @click="visichose()">
          </li>
        </ul>
      </header>
      <div class="visi_tit">
        <div class="tit_ico">
          <span class="image-com tit_font">{{this.teamname}}</span>
        </div>
        <div class="tit_yj">
      ...
      <div class="visi_sp">
        <ul class="">
          ...
          <li>
            <!--<span class="my_num">{{this.markinRrate}}<mt-spinner type="fading-circle"></mt-spinner></span>-->
            <span class="my_num">{{this.amountAverage}}<mt-spinner v-show="this.wait" type="fading-circle"></mt-spinner></span>
            <span class="my-uu">人均产能(万元)</span>
          </li>
        </ul>
      </div>
      ...
      <div class="description">
        ...
        <p class="description_p1 description_p2">年化业绩=规模业绩*期限/12；<br/>
          ...
          当期无到期单子时，续投率数据显示为——；<br/>
          当期有到期单但无续投时，续投率数据显示为0；<br/>
    ...
    <div class="popshow" v-if="popupShow">
        <div class="showmain" @click="nextchart()">
          <p class="showmain_b image-com">查看下级报表</p>
        </div>
        <div class="showmain2" @click="historychart()">
          <p class="showmain_p image-com">查看历史报表</p>
        </div>
    </div>
  </div>
</template>
...
export default {
  data () {
    return {
      ...
      chart:  "",
      active: '规模业绩',
      amountAverage: '',//团队当天人均产能
      teamcode: 0,
      chosee: true,
      weektime: [],//最近七天的日期
      investmentnum: [0, 0, 0, 0, 0, 0, 0],//最近七天规模业绩数据
      option: {},
      nowtime: '',//团队当天当日数据统计截止时间
      interestRateTest: '',//团队年化业绩 or 规模业绩
      intersYear: '',//团队年化业绩
      scaleYear: '',//团队规模业绩
      choseper: true,
      clesee: false,
      ...
      peopleRrate: '',//人均产能
      markinRrate: '',//团队折标率
      markingRateText: '',//团队续投率
      customerGrowthNum: '',//团队新客增长
      wait: true,
      interface: {
        scallinter: InterFace.scallinteam,//团队当天规模业绩，年化业绩，折标率
        investmenter: InterFace.investmenteam,//团队当天续投率
        newrater: InterFace.newrateam,//团队当天新客增长
        peopleRrater: InterFace.peopleRrater,//团队当天人均产能
        peopleRrater2: InterFace.peopleRrater2,//团队当月人均产能
        scallinter2: InterFace.scallinterteam2,//团队当月规模业绩，年化业绩，折标率
        investmenter2: InterFace.investmenterteam2,
        newrater2: InterFace.newraterteam2,
        weekscallinter: InterFace.weekscallinterteam,//一周规模业绩，年化业绩，折标率
        weeknewadd: InterFace.weeknewaddteam,//一周新客增长
        weekinvestmenter: InterFace.weekinvestmenteam,//续投率
      },
  ...
  watch: {
    ...
    '$route' (to, from) {
      ...
      if(to.name == 'visirank1' || from.name == 'visirank1') {
        ...
        this.people()
        //this.init()
        this.scale(this.interface.scallinter)//团队当天规模业绩，年化业绩，折标率
        this.investment(this.interface.investmenter)//团队当天续投率
        this.newrate(this.interface.newrater) //个人当天新客增长
        this.weekscale(this.interface.weekscallinter)//一周规模业绩，年化业绩，折标率
        this.interestRateTest_month = ''
        this.intersYear_month = ''
        this.scaleYear_month = ''
        this.markinRrate_month = ''
        this.markingRateText_month = ''
        this.customerGrowthNum_month = ''
        this.amountAverage_month = ''
        this.chosee = true
        this.clesee = false
      }
  ...
  created () {
    ...
    this.newrate(this.interface.newrater) //个人当天新客增长
    this.weekscale(this.interface.weekscallinter)//一周规模业绩，年化业绩，折标率
    this.peopleRra(this.interface.peopleRrater)
  },
  methods: {
    getStatus (urlStr) {
    ...
    common () {
      if (this.$route.params.code) {
        ...
        this.teamname = this.$route.params.deparname
        // console.log(this.params) //部门id
      } else {
        this.params = 0
        this.teamname = '团队'
      }
    ...
    selected (name) {
      ...
      if (name === '规模业绩') {
        ...
        this.dispeople(this.interface.weekscallinter)
      }
    },
    datechose () {
      this.chosee = true
      ...
      if (this.interestRateTest_today && this.intersYear_today) {
        ...
        this.amountAverage = this.amountAverage_today
        if (this.choseper == true) {
          this.interestRateTest = this.scaleYear_today
        } else {
          this.interestRateTest = this.intersYear_today
        }
      } else {
        //点击本日时重新请求数据
        this.scale(this.interface.scallinter)//团队当天规模业绩，年化业绩，折标率
        this.investment(this.interface.investmenter)//团队当天续投率
        this.newrate(this.interface.newrater) //团队当天新客增长
        this.peopleRra(this.interface.peopleRrater) //团队当天人均产能
      }
    },
    clechose () {
      //本月
      ...
      if (this.interestRateTest_month && this.intersYear_month) {
        ...
        this.amountAverage = this.amountAverage_month
        if (this.choseper == true) {
          this.interestRateTest = this.scaleYear_month
        } else {
          this.interestRateTest = this.intersYear_month
        }
      } else {
        //点击本月时重新请求数据
        this.scale(this.interface.scallinter2)
        this.investment(this.interface.investmenter2)  //本月续投率
        this.newrate(this.interface.newrater2) //新客增长
        this.peopleRra(this.interface.peopleRrater2)
      }
    },
    goBack () {
      this.$router.goBack()
    ...
    visichose () {
      // this.$router.push('/my/visteam/visichose')
      //this.$router.push('/my/visitcho1')
      //将部门id传过去
      // this.$router.push({ name: 'visicho1', params: {code: this.params} })
      this.popupShow = true
    },
    nextchart () {
    ...
    peopleRra (peopleRrater2) {
      let url_1 = 'api/services/app/userReport/apiGetTeamAverageProductivityToDay'
      var cha = (data) => {
        // console.log(data)
        this.amountAverage = data.result.amountAverage
        if (peopleRrater2 === url_1) {
          this.amountAverage_today = data.result.amountAverage
        } else {
          this.amountAverage_month = data.result.amountAverage
        }
      }
      Global.Ajax.post(Global.page.url.Request + peopleRrater2, this.cod, cha)
    },
    scale (scallinter) {
      let url_1 = 'api/services/app/userReport/apiGetTeamReportDay'  //本日
      var cha = (data) => {
        // console.log(data)
        this.wait = false
        ...
        if(this.choseper == true){
          ...
          this.interestRateTest = condata.interestRateTest
        }
        //年化业绩
        ...
        //将本日与本月分开存储   2018-9-4
        if (scallinter === url_1) {
          this.interestRateTest_today = condata.investmentAmountSumTest
          this.intersYear_today = condata.interestRateTest
          this.scaleYear_today = condata.investmentAmountSumTest
          this.markinRrate_today = condata.markinRrate
        } else {
          this.interestRateTest_month = condata.investmentAmountSumTest
          this.intersYear_month = condata.interestRateTest
          this.scaleYear_month = condata.investmentAmountSumTest
          this.markinRrate_month = condata.markinRrate
        }
      }
    ...
    people () {
      ...
      Global.Ajax.post(Global.page.url.Request + 'api/services/app/userReport/apiGetTeamAverageProductivityToDay', this.cod, cha)
    },
    performan () {
      //切换年化与规模业绩
    ...
    investment (investmenter) {
      let url_1 = 'api/services/app/userReport/apiGetTeamMarkingRate'//本日
      var cha = (data) => {
    ...
    newrate (newrater) {
      // console.log(newrater)
      let url_1 = 'api/services/app/userReport/apiGetNewCustomerTeamToDay' //本日
      var cha = (data) => {
    ...
    dispeople () {
      var cha = (data) => {
        ...
        let investment = []
        for (var i = 0; i < data.result.length; i++) {
          if (data.result[i].amountAverage === '--') {
            investment.push(0.00)
          } else {

            if(data.result[i].amountAverage.length>3){
              investment.push(parseFloat(data.result[i].amountAverage).toFixed(2))
            } else {
              investment.push(data.result[i].amountAverage)
            }
            console.log(data.result[i].amountAverage.length)
          }
        }
        this.investmentnum = investment
        setTimeout(()=>{
      ...
      Global.Ajax.post(Global.page.url.Request + 'api/services/app/userReport/apiGetTeamAverageProductivityToSevenDay', this.cod, cha)
    },
    ranking () {
      ...
      if (this.chosee === true) {
        if (this.choseper === true) {
          ...
          //本日规模业绩排名
          this.$router.push({ name: 'visirank1', params: {codtype: 2, visicod: this.params}})
        }
        if (this.choseper == false) {
          // alert("规模业绩")
          //本日年化业绩排名
          this.$router.push({ name: 'visirank1', params: {codtype: 1, visicod: this.params}})
        }
      }
      ...
      if (this.chosee === false) {
        if (this.choseper === true) {
          //规模业绩排名
          this.$router.push({ name: 'visirank1', params: {codtype: 4, visicod: this.params}})
        } else {
          //本月年化业绩排名
          this.$router.push({ name: 'visirank1', params: {codtype: 3, visicod: this.params}})
        }
      }
    }
