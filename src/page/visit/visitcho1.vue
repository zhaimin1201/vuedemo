...
export default {
  ...
  watch: {
    '$route' (to, from) {
      if(to.name === 'visicho2' || from.name === 'visicho2'  || to.name === 'Visteam' || from.name === 'Visteam') {
        //需要缓存
        this.lastparme = sessionStorage.getItem('lastID')
        console.log("yes")
    ...
    init () {
      ...
      peranking () {
        ...
        var cha = (data) => {
          ...
          console.log(this.ranking[0].isDepartment)
      ...
      subordinate (isDepartment, id) {
        ...
        if (isDepartment === false) {
          ...
          console.log(id)
          this.$router.push({ name: 'visicho2', params: {code: id} })
        }
      },
