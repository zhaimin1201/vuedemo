<template>
  <div class="calendarBox" style="overflow: hidden;height: 137px;    background: #F4F7F9">
    <div class="date">
      <span class="yearMonth left">{{now_year}}年{{now_month}}月
      ...
      <div class="calendar" @touchstart="moveStart" @touchend="moveEnd">
        <ul id="d2">
          <li :class="{black:item.isNowMonth,gray:!item.isNowMonth,red:item.checked,today:item.old.isToday}" v-for="(item,index) in list" @click="checkMe(item.id-1)">
            <div class="ite" style="font-size: 16px;position: relative; z-index: 2;">{{item.num}}</div><div style="font-size: 12px;position: relative; z-index: 2;">{{item.old.old_str}}</div>
          </li>
        </ul>
      </div>
    </div>
...
//              下个月的
          ...
          let time = String(this.now_year) + '-' + String(this.now_month) + '-' + String(this.oneMonthList[id].num)
          bus.$emit("sendeMsagToInhours", time)
